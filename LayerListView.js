...
define([
  ...
  './NlsStrings'
    ...
    CheckBox, PopupMenu, _OnDijitClickMixin, _TemplatedMixin, template,
    ...
    return declare([_WidgetBase, _TemplatedMixin], {
      ...
      postCreate: function() {
        // dojo.style(this.layerPanel,"visibility","hidden")
        var infoNode = domConstruct.create('div',{},this.informationPanel)
        infoNode.innerHTML = " "
        infoNode.innerHTML = this.config.description;
        // infoNode.innerHTML = "Hi I am Sahil. I am testing an information panel at this location. Below is a sample image <br> <br><img src='http://newcoastalatlas.tamug.edu/sahil/latest/TAMUPrimaryMarkALT.png' width=100%>"
        var clickHere = domConstruct.create('div',{'data-dojo-attach-event':'ondijitclick:_onClick'},infoNode)
        clickHere.innerHTML = "<br><button>Close</button><br><br>"

        // var clickHere2 = domConstruct.create('div',{'data-dojo-attach-event':'ondijitclick:_onClick'},infoNode)
        // clickHere2.innerHTML = "<br><button>Open</button><br><br>"

        // var nodes =  query(".layer-list-table")
        // array.forEach(nodes, function(node){
        //   console.log("this is a node")
        // })
        // query('.layer-list-table').style('visibility','visible');

        on(clickHere, "click", function(evt){
            
            domStyle.set(infoNode, "display", "none");
            console.log("making layerTable visibility visible")
            // domStyle.set("layerTable", "visibility", "visible"); //not being able to access layerTable i think when using the second same widget
            query('.layer-list-table').style('visibility','visible');
          });
        ...
        array.forEach(this.operLayerInfos.getLayerInfoArray(), function(layerInfo) {
          if(this.config.VisibleLayers.indexOf(layerInfo.title) == -1) return;
...
if (layerInfo.newSubLayers.length === 0) {
    ...
    addLayerNode: function(layerInfo, level, toTableNode) {
      ...
      divLabel2 = domConstruct.create('div', {
        'class': 'div-content',
        'style': 'display:inline'
      }, layerTitleTdNode);

       infoLink = domConstruct.create('a', {
        'href': layerInfo.layerObject.url,
        'target' : "_blank"
      }, divLabel2);


      var infoIconName = "images/get-info-15.png"
      infoIcon = domConstruct.create('img', {
        'src': this.layerListWidget.folderUrl + infoIconName,
        'width' : '17px',
